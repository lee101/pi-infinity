...
export interface AgentSessionConfig {
	...
	/** Automatically continue with concrete next steps after each completed run. */
	autoNextSteps?: boolean;
	/** Generate and implement a new improvement after current objectives finish. */
...
export class AgentSession {
	...
	private readonly _autoNextSteps: boolean;
	private readonly _autoNextIdea: boolean;
	...
	private async _handlePostAgentRun(): Promise<boolean> {
		...
		await this._maybeQueueAutonomousFollowUp(msg);
	...
	private async _maybeQueueAutonomousFollowUp(message: AssistantMessage): Promise<void> {
		if (
			(!this._autoNextSteps && !this._autoNextIdea) ||
		...
		const instructions = ["You are in AUTONOMOUS MODE. Do not ask for permission or confirmation."];
		if (this._autoNextSteps) {
			instructions.push(
				"Continue the current objective through its next concrete steps. Execute them in order and run relevant checks.",
			);
		}
		if (this._autoNextIdea) {
			instructions.push(
				"When the current objective is genuinely complete, identify three useful improvements, choose the highest-impact one, and begin implementing it.",
			);
		}
		if (this._autoNextSteps && this._autoNextIdea) {
			instructions.push("Repeat this cycle until a human interrupts you.");
		}

		await this.followUp(instructions.join("\n\n"));
	}
