...
Content`,
		...
		it("should resolve project paths relative to .pi", async () => {
			const extDir = join(tempDir, ".pinf", "extensions");
			mkdirSync(extDir, { recursive: true });
...
Content`,
			...
			mkdirSync(join(tempDir, ".pinf"), { recursive: true });
			...
			symlinkSync(sharedExtensionsDir, join(tempDir, ".pinf", "extensions"), "dir");
			symlinkSync(sharedSkillsDir, join(tempDir, ".pinf", "skills"), "dir");
			symlinkSync(sharedPromptsDir, join(tempDir, ".pinf", "prompts"), "dir");
			symlinkSync(sharedThemesDir, join(tempDir, ".pinf", "themes"), "dir");
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".pinf", "prompts");
			mkdirSync(promptsDir, { recursive: true });
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should keep ~/.agents/skills user-scoped when cwd is under home in a non-git directory", async () => {
			...
			try {
				...
				const localAgentDir = join(tempDir, ".pinf", "agent");
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .pi auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".pinf\n");

			const skillDir = join(tempDir, ".pinf", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("npmCommand", () => {
		...
		it("should update git package dependencies with --omit=dev", async () => {
			const source = "git:github.com/user/repo";
			const targetDir = join(tempDir, ".pinf", "git", "github.com", "user", "repo");
			mkdirSync(targetDir, { recursive: true });
			writeFileSync(join(targetDir, "package.json"), JSON.stringify({ name: "repo", version: "1.0.0" }));
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .pi settings base", () => {
			...
			const rel = relative(join(tempDir, ".pinf"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
	...
	describe("offline mode and network timeouts", () => {
		it("should update project npm packages using @latest when newer version is available", async () => {
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"npm",
				["install", "example@latest", "--prefix", join(tempDir, ".pinf", "npm")],
				undefined,
			);
		});
		...
		it("should batch npm updates per scope and run git updates in parallel while skipping pinned and current packages", async () => {
			...
			const projectOldPath = join(tempDir, ".pinf", "npm", "node_modules", "project-old");
			const projectCurrentPath = join(tempDir, ".pinf", "npm", "node_modules", "project-current");
			...
			expect(runCommandSpy).toHaveBeenNthCalledWith(
				2,
				"npm",
				["install", "project-old@latest", "project-missing@latest", "--prefix", join(tempDir, ".pinf", "npm")],
				undefined,
			);
		...
		it("should skip pinned packages when checking for updates", async () => {
			const installedNpmPath = join(tempDir, ".pinf", "npm", "node_modules", "example");
